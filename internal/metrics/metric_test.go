...
func TestRecordMountRequest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			RecordMountRequest(tt.status, tt.duration, tt.ctx)
...
func TestRecordObjectProcessed(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			RecordObjectProcessed(tt.objectType, tt.status, tt.ctx)
...
func TestUpdateSpireConnectionStatus(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			UpdateSpireConnectionStatus(tt.connected, tt.ctx)

			value := testutil.ToFloat64(SpireConnectionStatus.WithLabelValues(
				tt.ctx.Namespace, tt.ctx.ServiceAccount, tt.ctx.PodUID,
			))
...
func TestRecordSpireConnectionAttempt(t *testing.T) {
	...
	RecordSpireConnectionAttempt(ctx)

	count := testutil.ToFloat64(SpireConnectionAttempts.WithLabelValues(
		ctx.Namespace, ctx.ServiceAccount, ctx.PodUID,
	))
	if count < 1 {
		t.Error("Counter should be incremented")
	}
...
func TestRecordSpireSVIDReceived(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			RecordSpireSVIDReceived(tt.svidType, tt.ctx)

			count := testutil.ToFloat64(SpireSVIDsReceived.WithLabelValues(
				tt.svidType, tt.ctx.Namespace, tt.ctx.ServiceAccount, tt.ctx.PodUID,
			))
			if count < 1 {
				t.Error("Counter should be incremented")
			}
...
func TestUpdateCacheSizes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			UpdateCacheSizes(tt.jwtCacheSize, tt.svidCacheSize, tt.ctx)
...
func TestProviderPoolMetrics(t *testing.T) {
	...
	// Test pool hit
	RecordProviderPoolHit("example.org", "default", "test-sa")
	...
	// Test pool miss
	RecordProviderPoolMiss("example.org", "production", "api-sa")
...
func TestMetrics_ConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		go func() {
			RecordMountRequest("success", 0.1, ctx)
			done <- true
		}()
	}
	...
	for i := 0; i < 20; i++ {
		go func() {
			RecordObjectProcessed("x509-svid", "success", ctx)
			done <- true
		}()
	}
	...
	for i := 0; i < 20; i++ {
		go func() {
			UpdateSpireConnectionStatus(true, ctx)
			done <- true
		}()
	}
